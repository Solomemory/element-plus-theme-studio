...
The online preview is deployed as a static GitHub Pages site. It supports theme editing, presets, random themes, dark mode preview, and JSON import/export. ZIP generation requires the local Node builder, so run `pnpm dev` or `pnpm theme:build` when you need to generate a complete theme package.
