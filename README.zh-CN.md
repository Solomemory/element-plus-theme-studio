...
- 可视化编辑 Element Plus 主题 Token，包括颜色、文字、边框、背景、圆角、阴影、尺寸、显示密度、HTML 字号、Sass 变量和 CSS 覆盖
- 左侧 Token 表单按 Tab 分类展示，编辑面板和预览区域独立滚动
- 实时预览常用 Element Plus 组件，覆盖表单、操作、反馈、导航、数据展示、内容和弹窗类组件
- 支持亮色 / 暗黑主题预览，并生成独立的 `dark.css`
- 支持中文 / English 界面切换
- 内置多套主题预设，包括 Aura Blue、Glass Aurora、库比蒂诺极简、Data Wall、新野兽派、软陶彩块、柔和拟物、极简刊物、Bento 薄荷、扁平糖果、暗夜霓虹、怪盗红、翡翠控制台、蔷薇石英、石墨专业版、Vben Admin 等
- 支持一键生成随机主题，快速探索配色方向
- 支持导入 / 导出 `tokens.json`
...
2. 在左侧 Token 面板中编辑显示密度、颜色、圆角、背景、文字、边框、HTML 字号、Sass 变量和 CSS 覆盖。
...
`density` 用来控制编辑器的语义化尺寸模式：`compact`、`default`、`comfortable`、`large`。切换密度时，会一起调整 `typography.htmlFontSize`、Element Plus 字号 token 和组件高度 token。

`typography.htmlFontSize` 仍然作为高级根字号 token 保留，并同步写入 `--studio-root-font-size` / `--studio-html-font-size`。`--studio-font-size-base` / `--font-size-base` 会对应 `typography.base`。

推荐的尺寸策略：

- 用 `density` 作为系统 UI 尺寸的主控制。
- Element Plus 字号、组件高度、边框和细线细节保持 `px`，表单、表格和弹窗更稳定。
- `htmlFontSize` 只作为高级根字号基准，适合已经使用 `rem` 体系的业务项目。
- `rem` 更适合放在业务布局或自定义内容 CSS 中，不建议作为所有 Element Plus token 的默认单位。
